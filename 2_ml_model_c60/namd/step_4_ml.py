...
for step in range(istep,fstep):
    energy_filename = F"{args.functional}_size_{args.size}/Hvib_sd_{step}_re.npz"
...
St = []
for step in range(istep,fstep):        
    St_filename = F"{args.functional}_size_{args.size}/St_sd_{step}_re.npz"
...
for c, step in enumerate(range(istep,fstep)):
    nac_filename = F"{args.functional}_size_{args.size}/Hvib_sd_{step}_im.npz"
...
#===== St = np.load(F'../ml_{args.functional}_time_overlap_size_{args.size}.npy')[:,119:150,119:150]
#===== E1 = np.load(F'../{args.functional}_ml_energies_size_{args.size}.npy')[:,119:150]
#===== E = []
...
dyn_general = { "nsteps":NSTEPS, "ntraj":args.ntraj, "nstates":NSTATES, "dt":1.0*units.fs2au,
...
def function1(icond):
    ...
    dyn_gen.update({"prefix":F"{prf}_functional_{args.functional}_size_{args.size}_istate_{istate}_icond_{icond}", "prefix2":F"{prf}_functional_{args.functional}_size_{args.size}_istate_{istate}_icond_{icond}" }) 
...
ICONDS = list(np.linspace(1,1001,nthreads, endpoint=False, dtype=int))
################################
