...
atomic_ks_mats = np.load('../1_data_generation/1_c60/c60_pbe_atomic_ks_mats.npy')
converged_ks_mats = np.load(F'../1_data_generation/1_c60/c60_{functional}_converged_ks_mats.npy')
atomic_overlaps = np.load('../1_data_generation/1_c60/c60_pbe_atomic_overlaps.npy')
converged_overlaps = np.load(F'../1_data_generation/1_c60/c60_{functional}_converged_overlaps.npy')
atomic_coeffs = np.load('../1_data_generation/1_c60/c60_pbe_atomic_coeffs.npy')
converged_coeffs = np.load(F'../1_data_generation/1_c60/c60_{functional}_converged_coeffs.npy')
...
for i in range(nsplit-1):
    ...
    mse = np.sqrt( mean_squared_error(inputs_test[i], outputs_test[i]))
...
np.save(F'train_time_{functional}_size_{size}.npy', np.array([time.time()-t1]))
...
filename = '../1_data_generation/1_c60/3_converged_wfn/c60_1000-RESTART.wfn'
...
for step in range(len(converged_ks_mats_1)):
    ...
    production_time.append(time.time()-t1)
    t1 = time.time()
    ...
    #print(conv_coeff[0])
    #print(eigenvectors)
    print('Converged HOMO-LUMO gap:', (eigenvalues_converged[120]-eigenvalues_converged[119])*units.au2ev)
    print('ML HOMO-LUMO gap:', (eigenvalues[120]-eigenvalues[119])*units.au2ev)
    print('Atomic guess (PBE guess) HOMO-LUMO gap:', (eigenvalues_atomic[120]-eigenvalues_atomic[119])*units.au2ev)
    #if step==shuffled_indices[-1]:
    output_name = F'ml_{functional}_wfns/ml_c60_{functional}_size_{size}_{step+istep}-RESTART.wfn' 
    CP2K_methods.write_wfn_file(output_name, basis_data, spin_data, eigen_vals_and_occ_nums, [eigenvectors[0:150,:]])
    error_ml = np.average(np.abs(np.abs(eigenvectors)[0:150,:] - np.abs(conv_coeff[0])))
...
np.save(F'production_time_{functional}_size_{size}.npy', production_time)
